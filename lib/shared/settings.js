...
export class ConfigManager extends GObject.Object {
  ...
  // Workspace placement configuration (separate from window overrides)
  get defaultWorkspaceConfigFile() {
    const defaultConfig = GLib.build_filenamev([this.extensionPath, `config`, `workspaces.json`]);
    const defaultConfigFile = Gio.File.new_for_path(defaultConfig);
    if (defaultConfigFile.query_exists(null)) {
      return defaultConfigFile;
    }
    return null;
  }

  loadDefaultWorkspaceConfigContents() {
    const defaultFile = this.defaultWorkspaceConfigFile;
    if (defaultFile) {
      const contents = this.loadFileContents(defaultFile);
      if (contents) {
        return JSON.parse(contents);
      }
    }
  ...
  get workspaceConfigFile() {
    const profileSettingPath = `${this.confDir}/config`;
    const settingFile = "workspaces.json";
    const defaultSettingFile = this.defaultWorkspaceConfigFile;
    return this.loadFile(profileSettingPath, settingFile, defaultSettingFile);
  }

  get workspaceProps() {
    let configFile = this.workspaceConfigFile;
    if (!configFile) {
      configFile = this.defaultWorkspaceConfigFile;
    }
    if (!configFile) {
      return { rules: [] };
    }
    ...
    if (success) {
      const configContents = imports.byteArray.toString(contents);
      return JSON.parse(configContents);
    }
    return { rules: [] };
  }

  set workspaceProps(props) {
    // Always write to user config directory, not the extension default
    const userConfigPath = `${this.confDir}/config`;
    const configFilePath = GLib.build_filenamev([userConfigPath, "workspaces.json"]);
    const configFile = Gio.File.new_for_path(configFilePath);

    let configContents = JSON.stringify(props, null, 4);
    const PERMISSIONS_MODE = 0o744;

    if (GLib.mkdir_with_parents(userConfigPath, PERMISSIONS_MODE) === 0) {
