...
export class WindowManager extends GObject.Object {
  ...
  getWorkspacePlacementRule(metaWindow) {
    ...
    for (const override of this.windowProps.overrides) {
      ...
      Logger.debug(
        `  Checking rule: wmClass="${override.wmClass}", workspace=${override.workspace}`
      );
      ...
      Logger.info(`Workspace rule matched: ${wmClass} -> workspace ${override.workspace}`);
  ...
  findParentApplicationWorkspace(metaWindow) {
    ...
    for (const nodeWindow of windows) {
      ...
      if (nodeWindow.nodeValue.get_wm_class() === wmClass) {
        const workspace = nodeWindow.nodeValue.get_workspace();
        if (workspace) {
          Logger.debug(`Parent found on other monitor ${nodeWindow.nodeValue.get_monitor()}`);
          return {
            monitor: nodeWindow.nodeValue.get_monitor(),
            workspace: workspace.index(),
          };
