...
export class WindowManager extends GObject.Object {
  ...
  toggleFloatingMode(action, metaWindow) {
    ...
    if (floatingExempt) {
      ...
      // Set user intent to TILE
      nodeWindow.userFloatIntent = false;
      ...
      // Set user intent to FLOAT
      nodeWindow.userFloatIntent = true;
      nodeWindow.mode = WINDOW_MODES.FLOAT;
    }
  }
    ...
    settings.connect("changed", (_, settingName) => {
      switch (settingName) {
        case "window-overrides-reload-trigger":
          ...
          this.reloadWindowOverrides();
          break;
        case "workspace-rules-reload-trigger":
          this.reloadWorkspaceRules();
          break;
        case "workspace-reorganize-trigger":
  ...
  command(action) {
    ...
    switch (action.name) {
      ...
      case "FloatClassToggle":
        if (!focusWindow) {
          Logger.warn("FloatClassToggle: No focused window, ignoring command");
          return;
        }

        this.toggleFloatingMode(action, focusWindow);

        if (focusNodeWindow.mode === WINDOW_MODES.FLOAT) {
          const rectRequest = {
            x: action.x,
            y: action.y,
            width: action.width,
        ...
        this.queueEvent({
          ...
          callback: () => {
            if (this.eventQueue.length <= 0) {
              this.unfreezeRender();
              // Fix: Check both focusNodeWindow and parentNode before accessing
              if (
                focusNodeWindow &&
      ...
      case "LayoutTabbedToggle":
        ...
        Logger.debug(
          `LayoutTabbedToggle: focus window parent is ${focusNodeWindow.parentNode.nodeType}`
        );

        // Check if parent is MONITOR (top-level windows)
        if (focusNodeWindow.parentNode.isMonitor()) {
          let monitor = focusNodeWindow.parentNode;
          let topLevelNodes = monitor.childNodes;

          Logger.debug(`LayoutTabbedToggle: top-level with ${topLevelNodes.length} nodes`);

          // If multiple top-level nodes, group them all in a tabbed container
          if (topLevelNodes.length > 1) {
            this.tree.groupNodesInTabbedContainer(monitor, focusNodeWindow);
          } else {
            // Single window: create container as before
            Logger.debug(`LayoutTabbedToggle: single window, using split`);
            this.tree.split(focusNodeWindow, ORIENTATION_TYPES.HORIZONTAL, true);
            focusNodeWindow.parentNode.layout = LAYOUT_TYPES.TABBED;
            focusNodeWindow.parentNode.lastTabFocus = focusNodeWindow.nodeValue;
          }
        } else {
          // Window already in container: toggle container layout
          currentLayout = focusNodeWindow.parentNode.layout;

          Logger.debug(`LayoutTabbedToggle: window in container, current layout=${currentLayout}`);

          if (currentLayout === LAYOUT_TYPES.TABBED) {
            // Toggle off: restore split layout
            Logger.debug(`LayoutTabbedToggle: toggling off tabbed mode`);
            focusNodeWindow.parentNode.layout = this.determineSplitLayout();
            this.tree.resetSiblingPercent(focusNodeWindow.parentNode);
            focusNodeWindow.parentNode.lastTabFocus = null;
          } else {
            // Toggle on: set to tabbed
            Logger.debug(`LayoutTabbedToggle: toggling on tabbed mode`);
            focusNodeWindow.parentNode.layout = LAYOUT_TYPES.TABBED;
            focusNodeWindow.parentNode.lastTabFocus = focusNodeWindow.nodeValue;
          }
        }

        // Restore focus after tree manipulation
        focusNodeWindow.nodeValue.raise();
        focusNodeWindow.nodeValue.focus(global.display.get_current_time());
        focusNodeWindow.nodeValue.activate(global.display.get_current_time());
        Logger.debug(`LayoutTabbedToggle: focus restored to window`);
      ...
      case "NukeOrphanedDecorations":
        // Emergency cleanup command to remove all orphaned decorations
        // This is a safety net for when decorations persist after container removal
  ...
  renderTree(from, force = false) {
    ...
    if (this._freezeRender || !this.ext.settings.get_boolean("tiling-mode-enabled")) {
      ...
      // Cancel any pending render to ensure we use the latest state
      if (this._renderTreeSrcId) {
        GLib.source_remove(this._renderTreeSrcId);
        this._renderTreeSrcId = 0;
      }
  ...
  processFloats() {
    this.allNodeWindows.forEach((nodeWindow) => {
      ...
      if (nodeWindow.userFloatIntent !== null) {
        nodeWindow.mode = nodeWindow.userFloatIntent ? WINDOW_MODES.FLOAT : WINDOW_MODES.TILE;
        return;
      }

      // Auto-detection for windows without explicit user intent
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        // Log current state - using error level to ensure it shows
        const currentMonitor = global.display.get_current_monitor();
        const activeWorkspace = global.workspace_manager.get_active_workspace_index();
        Logger.error(
          `=== NEW WINDOW: ${metaWindow.get_title()} (${metaWindow.get_wm_class()}) ===`
        );
        Logger.error(`CURRENT MONITOR: ${currentMonitor}, ACTIVE WORKSPACE: ${activeWorkspace}`);

        // Determine target workspace using priority rules
        const targetPlacement = this.determineTargetWorkspace(metaWindow);
        const targetMonitor = targetPlacement.monitor;
        let targetWorkspace = targetPlacement.workspace;

        Logger.error(`TARGET: monitor=${targetMonitor}, workspace=${targetWorkspace}`);

        // Validate workspace exists
        const wsManager = global.workspace_manager;
        if (targetWorkspace >= wsManager.get_n_workspaces()) {
          Logger.warn(`Invalid workspace ${targetWorkspace}, using current`);
          targetWorkspace = wsManager.get_active_workspace_index();
        }

        let metaMonWs = `mo${targetMonitor}ws${targetWorkspace}`;

        Logger.error(`FINAL: ${metaMonWs}`);

        // Check if the target monitor / workspace exists
        ...
        // Set attach target to target workspace node
        attachTarget = metaMonWsNode;

        // If there are windows in target workspace, attach to first window
        ...
        // Move window to target monitor if GNOME placed it elsewhere
        this.moveWindowToTargetMonitor(metaWindow, targetMonitor);
  ...
  updateDecorationLayout() {
    ...
    monWsNoMaxWindows.forEach((monitorWs) => {
      ...
      activeMonWsCons.forEach((con) => {
        ...
        // DEBUG_DECORATION: Window.js decoration check
        Logger.info(
          "DEBUG_DECORATION: Container check - decoration: " +
            !!con.decoration +
            ", tiled: " +
            tiled.length +
            ", showTabs: " +
        ...
        if (con.decoration && tiled.length > 0 && showTabs) {
          Logger.info("DEBUG_DECORATION: Attempting to show decoration in updateDecorationLayout");

          try {
            con.decoration.show();
            Logger.info("DEBUG_DECORATION: Decoration shown successfully");

            if (global.window_group.contains(con.decoration) && this.focusMetaWindow) {
              let focusActor = this.focusMetaWindow.get_compositor_private();
              // Validate focus actor before insertion
              if (focusActor && focusActor.get_stage() !== null) {
                global.window_group.remove_child(con.decoration);
                // Show it below the focused window
                global.window_group.insert_child_below(con.decoration, focusActor);
                Logger.info("DEBUG_DECORATION: Decoration repositioned below focused window");
              } else {
                Logger.warn("DEBUG_DECORATION: Focus actor invalid, decoration not repositioned");
              }
            } else {
              Logger.warn("DEBUG_DECORATION: Decoration not in window group or no focused window");
            }
          } catch (e) {
            Logger.warn("DEBUG_DECORATION: Failed to show decoration: " + e.message);
            con.decoration = null;
          }
      ...
      if (!preview) {
        ...
        if (childNode.createCon) {
          ...
          if (isLeft || isRight) {
            ...
            // Preserve existing tabbed/stacked layout when adding to those containers
            if (stackedOrTabbed) {
              // Container is already tabbed/stacked, maintain its layout
              // No change needed - layout is already correct
            } else if (containerNode.isHSplit() || containerNode.isVSplit()) {
              // Convert HSPLIT/VSPLIT to tabbed/stacked based on centerLayout setting
  ...
  isFloatingExempt(metaWindow) {
    ...
    // "tile" overrides take precedence over all auto-float detection.
    // This allows users to force-tile windows that report allows_resize()=false
    // (e.g., Chromium/Electron apps on Wayland).
    const tileOverride =
      this.windowProps.overrides.filter(
        (o) => o.mode === "tile" && o.wmClass && o.wmClass.includes(wmClass)
      ).length > 0;
  ...
  /**
   * Reload workspace placement rules from the separate configuration file
   */
  reloadWorkspaceRules() {
    const freshProps = this.ext.configMgr.workspaceProps;
    this.workspaceRules = freshProps?.rules || [];
    Logger.info(`Reloaded ${this.workspaceRules.length} workspace rules from file`);
  }
  ...
  getWorkspacePlacementRule(metaWindow) {
    ...
    const wmTitle = metaWindow.get_title();

    Logger.debug(`Checking workspace rules for: wmClass="${wmClass}", wmTitle="${wmTitle}"`);

    if (!wmClass) return null;

    // Use separate workspace rules config
    const rules = this.workspaceRules || [];
    for (const rule of rules) {
      if (rule.workspace === undefined) continue;

      Logger.debug(`  Checking rule: wmClass="${rule.wmClass}", workspace=${rule.workspace}`);

      if (rule.wmClass !== wmClass) continue;

      if (rule.wmTitle) {
        if (!wmTitle) continue;
        const titles = rule.wmTitle.split(",");
        const matches = titles.some((t) => {
          if (t.startsWith("!")) {
            return !wmTitle.includes(t.slice(1));
          }
          return wmTitle.includes(t);
        });
        if (!matches) continue;
      }

      // Config stores 1-based workspace, convert to 0-based for runtime
      const wsIndex = rule.workspace - 1;
      Logger.info(
        `Workspace rule matched: ${wmClass} -> workspace ${rule.workspace} (index ${wsIndex})`
      );
      return { workspace: wsIndex };
    }

    Logger.debug(`No workspace rule matched for ${wmClass}`);
    return null;
  }

  /**
   * Get monitor index from tree structure by walking up to MONITOR node
   */
  getMonitorIndexFromNode(nodeWindow) {
    let node = nodeWindow;
    while (node) {
      if (node.nodeType === NODE_TYPES.MONITOR) {
        return Utils.monitorIndex(node.nodeValue);
      }
  ...
  findParentApplicationWorkspace(metaWindow) {
    ...
    // First pass: look for parent on current monitor (using tree structure)
    for (const nodeWindow of windows) {
      if (nodeWindow.nodeValue === metaWindow) continue;
      if (nodeWindow.nodeValue.get_wm_class() === wmClass) {
        // Use tree structure to determine monitor, not Meta.Window
        const nodeMonitor = this.getMonitorIndexFromNode(nodeWindow);
        if (nodeMonitor === currentMonitor) {
          const workspace = nodeWindow.nodeValue.get_workspace();
          if (workspace) {
            Logger.debug(
              `Parent found on current monitor ${currentMonitor} (tree: ${nodeMonitor})`
            );
            return {
              monitor: nodeMonitor,
    ...
    // Second pass: look for parent on any monitor
    for (const nodeWindow of windows) {
      if (nodeWindow.nodeValue === metaWindow) continue;
      if (nodeWindow.nodeValue.get_wm_class() === wmClass) {
        const workspace = nodeWindow.nodeValue.get_workspace();
        if (workspace) {
          const nodeMonitor = this.getMonitorIndexFromNode(nodeWindow);
          Logger.debug(`Parent found on other monitor ${nodeMonitor}`);
          return {
            monitor: nodeMonitor,
            workspace: workspace.index(),
          };
  ...
  determineTargetWorkspace(metaWindow) {
    Logger.debug(`=== Determining target workspace for ${metaWindow.get_wm_class()} ===`);

    const rule = this.getWorkspacePlacementRule(metaWindow);
    if (rule) {
      const monitor = global.display.get_current_monitor();
      Logger.info(`Using workspace rule: monitor=${monitor}, workspace=${rule.workspace}`);
      return {
        monitor: monitor,
    ...
    if (parent) {
      Logger.info(
        `Using parent workspace: monitor=${parent.monitor}, workspace=${parent.workspace}`
      );
      return parent;
    }
    ...
    Logger.info(`Using fallback: monitor=${fallback.monitor}, workspace=${fallback.workspace}`);
    return fallback;
  }

  /**
   * Move window to target monitor if it's on a different monitor.
   * This ensures Forge controls initial window placement rather than
   * just tracking where GNOME/Mutter places windows.
   */
  moveWindowToTargetMonitor(metaWindow, targetMonitor) {
    ...
    const workarea = global.display
      ...
      .get_work_area_for_monitor(targetMonitor);

    if (workarea) {
      Logger.info(
        `Moving window from monitor ${currentMonitor} to ${targetMonitor} at (${workarea.x}, ${workarea.y})`
      );
  ...
  /**
   * Reorganize all windows according to workspace placement rules
   */
  reorganizeWindowsByWorkspaceRules() {
    Logger.info("Reorganizing windows by workspace rules");
    this.reloadWorkspaceRules();
    ...
    for (const nodeWindow of windows) {
      ...
      const currentMonitor = metaWindow.get_monitor();
      const needsWorkspaceChange = currentWs.index() !== rule.workspace;
      const needsMonitorChange = currentMonitor !== primaryMonitor;

      if (needsWorkspaceChange || needsMonitorChange) {
        // Move to primary monitor if on secondary
        if (needsMonitorChange) {
          const workarea = global.display
            .get_workspace_manager()
            .get_active_workspace()
          ...
          metaWindow.move_frame(true, workarea.x, workarea.y);
          Logger.info(
            `Moved ${metaWindow.get_wm_class()} from monitor ${currentMonitor} to primary monitor ${primaryMonitor}`
          );
        }

        // Change workspace if needed
        if (needsWorkspaceChange) {
          const targetWs = global.workspace_manager.get_workspace_by_index(rule.workspace);
          if (targetWs) {
            metaWindow.change_workspace(targetWs);
            Logger.info(
              `Moved ${metaWindow.get_wm_class()} from workspace ${currentWs.index()} to ${
                rule.workspace
              }`
            );
          }
    ...
    Logger.info(`Reorganization complete: moved ${movedCount} windows`);
    this.renderTree("reorganize-workspaces");
  }
}
