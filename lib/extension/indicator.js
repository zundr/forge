...
export class ReorganizeButton extends PanelMenu.Button {
  static {
    GObject.registerClass(this);
  }

  constructor(extension) {
    super(0.0, _("Reorganize Windows"), true);
    this.extension = extension;
    ...
    this.connect("button-press-event", () => {
      this.extension.settings.set_uint("workspace-reorganize-trigger", Date.now());
      return false;
    });

    this.visible = this.extension.settings.get_boolean("panel-reorganize-button-enabled");

    this.extension.settings.connect("changed::panel-reorganize-button-enabled", () => {
      this.visible = this.extension.settings.get_boolean("panel-reorganize-button-enabled");
    });
  }
}
