...
import { trackParent, isChildOf } from "./safe-widget.js";
...
export class Tree extends Node {
  ...
  groupNodesInTabbedContainer(parentNode, focusNode) {
    if (!parentNode || !focusNode) return;

    Logger.debug(
      `groupNodesInTabbedContainer: parent=${parentNode.nodeType}, children=${parentNode.childNodes.length}`
    );

    // Create new tabbed container
    let container = new St.Bin();
    let tabbedCon = new Node(NODE_TYPES.CON, container);
    tabbedCon.settings = this.settings;
    tabbedCon.layout = LAYOUT_TYPES.TABBED;
    tabbedCon.lastTabFocus = focusNode.nodeValue;

    // Copy rect and percent from focus node
    tabbedCon.rect = focusNode.rect;
    tabbedCon.percent = focusNode.percent || 1.0;

    // Get all current children (copy array since we'll modify parent)
    let children = [...parentNode.childNodes];
    Logger.debug(
      `groupNodesInTabbedContainer: moving ${children.length} children into tabbed container`
    );

    // Insert tabbed container at the position of the first child
    let firstChildIndex = children[0].index;
    parentNode.insertBefore(tabbedCon, children[0]);

    // Move all children into tabbed container using appendChild
    // appendChild will properly call removeChild on the old parent
    children.forEach((child) => {
      Logger.debug(`groupNodesInTabbedContainer: moving child ${child.nodeType}`);
      tabbedCon.appendChild(child);
    });

    this.attachNode = tabbedCon;
    Logger.debug(`groupNodesInTabbedContainer: complete, attachNode set to tabbed container`);
  }
    ...
    if (node.nodeType === NODE_TYPES.MONITOR || node.nodeType === NODE_TYPES.CON) {
      ...
      if (decoration) {
        try {
          ...
          // Decoration was disposed between check and usage - clear reference
          node.decoration = null;
        }
      }
    ...
    if (layout === LAYOUT_TYPES.TABBED) {
      ...
      if (node.childNodes.length > 1 || alwaysShowDecorationTab) {
        ...
        if (node.decoration && child.isWindow()) {
          // Recreate decoration if it was disposed by GNOME Shell GC
          try {
            void node.decoration.visible;
          } catch (e) {
            node.decoration = null;
          ...
          try {
            ...
            Logger.warn(`Border theme node disposed: ${e.message}`);
          }
          ...
          // Use try-catch at point of use to handle TOCTOU race with GC disposal.
          // Any isDisposed() check-then-use pattern is vulnerable because GC can
          // dispose the object between the check and the subsequent method call.
          try {
            node.decoration.set_size(adjustWidth, params.stackedHeight);
            ...
            if (child.tab) {
              // JS-side parent tracking eliminates native .contains() call
              // which triggers uncatchable GLib warnings on disposed objects.
            ...
            Logger.warn(`Decoration disposed during render: ${e.message}`);
            node.decoration = null;
          }
