...
/**
 * Check if a GObject (like St.Widget) has been disposed.
 * Disposed objects are not null but accessing them causes crashes.
 */
function isDisposed(obj) {
  if (!obj) return true;
  try {
    // Accessing any property on a disposed GObject throws
    void obj.visible;
    return false;
  } catch (e) {
    return true;
...
export class Node extends GObject.Object {
  ...
  constructor(type, data) {
    ...
    this.app = null;
    this.pointer = null;
    // User intent for float mode: null = auto, true = user wants float, false = user wants tile
    this.userFloatIntent = null;
  ...
  removeChild(node) {
    ...
    if (this.contains(node)) {
      ...
      let removed = parentNode.childNodes.splice(node.index, 1);
      refNode = removed[0]; // splice returns array, get first element
      if (refNode) {
  ...
  _createWindowTab() {
    // DEBUG_TAB: Tab creation start
    Logger.info(
      "DEBUG_TAB: Creating tab for window: " +
        (this.nodeValue ? this.nodeValue.get_title() : "unknown")
    );

    if (this.tab || !this.isWindow()) {
      Logger.info(
        "DEBUG_TAB: Early return - tab exists: " + !!this.tab + ", isWindow: " + this.isWindow()
      );
      return;
    }
    ...
    // DEBUG_TAB: Tab widget creation
    Logger.info("DEBUG_TAB: Creating tab widgets - app: " + !!app + ", title: " + labelText);
    ...
    if (metaWin === global.display.get_focus_window()) {
      tabContents.add_style_class_name("window-tabbed-tab-active");
      Logger.info("DEBUG_TAB: Tab marked as active");
    }
    this.tab = tabContents;
    Logger.info("DEBUG_TAB: Tab creation complete, tab object: " + !!this.tab);
  }

  _createDecoration() {
    // DEBUG_DECORATION: Decoration creation start
    Logger.info("DEBUG_DECORATION: Creating decoration for container");

    if (this.decoration) {
      Logger.info("DEBUG_DECORATION: Decoration already exists, skipping");
      return;
    }
    ...
    Logger.info("DEBUG_DECORATION: Created BoxLayout, adding to window group");

    if (!globalWinGrp.contains(decoration)) {
      globalWinGrp.add_child(decoration);
      Logger.info("DEBUG_DECORATION: Added decoration to window group");
    } else {
      Logger.info("DEBUG_DECORATION: Decoration already in window group");
    }

    decoration.hide();
    this.decoration = decoration;
    Logger.info("DEBUG_DECORATION: Decoration creation complete, hidden by default");
  }
...
export class Tree extends Node {
  ...
  split(node, orientation, forceSplit = false) {
    ...
    // Mark as explicit split to prevent condensing
    newConNode.explicitSplit = true;

    // Clear the flag after 5 seconds to allow future condensing
    GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 5, () => {
      newConNode.explicitSplit = false;
  ...
  cleanTree() {
    ...
    // Phase 3: condense redundant nested containers with same layout
    let hasCondensed = false;
    if (this.extWm.ext.settings.get_boolean("condense-redundant-containers")) {
      hasCondensed = this._condenseRedundantContainers();
    }

    if (hasOrphanCons || invalidWindows.length > 0 || hasCondensed) {
      this.processNode(this);
      this.apply(this);
    }
  }
  ...
  /**
   * Condense redundant nested containers with same layout type
   * Example: MONITOR [HSPLIT] → CON [HSPLIT] becomes MONITOR [HSPLIT] with children moved up
   * @returns {boolean} True if any containers were condensed
   */
  _condenseRedundantContainers() {
    let condensed = false;
    const allNodes = this.getNodeByType(NODE_TYPES.CON);

    allNodes.forEach((node) => {
      const parent = node.parentNode;

      // Check if this container is redundant:
      // 1. Parent exists and has a layout
      // 2. Parent and child have same layout type
      // 3. Container was not explicitly created by user
      // 4. Parent is not ROOT (structural node)
      if (
        parent &&
        parent.layout &&
        parent.layout === node.layout &&
        !node.explicitSplit &&
        parent.nodeType !== NODE_TYPES.ROOT
      ) {
        Logger.debug(`Condensing redundant ${node.layout} container`);
        ...
        // Remove the now-empty redundant container
  ...
  processNode(node) {
    ...
    if (node.nodeType === NODE_TYPES.MONITOR || node.nodeType === NODE_TYPES.CON) {
      ...
      // DEBUG_SETTINGS: Settings and layout info
      Logger.info(
        "DEBUG_SETTINGS: showTabs=" +
          showTabs +
          ", stackedHeight=" +
      ...
      if (decoration) {
        // Check if decoration was disposed
    ...
    if (layout === LAYOUT_TYPES.TABBED) {
      ...
      if (node.childNodes.length > 1 || alwaysShowDecorationTab) {
        ...
        if (node.decoration && child.isWindow()) {
          // Check if decoration was disposed and recreate if needed
          if (isDisposed(node.decoration)) {
            Logger.warn("DEBUG_DECORATION: Decoration was disposed, recreating");
            node.decoration = null;
            node._createDecoration();
          }
          ...
          if (decoration !== null && decoration !== undefined) {
            try {
              // DEBUG_DECORATION: Decoration positioning and sizing
              Logger.info(
                "DEBUG_DECORATION: Setting decoration size: " +
                  adjustWidth +
                  "x" +
                  params.stackedHeight +
                  " at " +
              ...
              if (params.tiledChildren.length > 0 && params.stackedHeight !== 0) {
                decoration.show();
                Logger.info(
                  "DEBUG_DECORATION: Decoration shown - tiledChildren: " +
                ...
                decoration.hide();
                Logger.info(
                  "DEBUG_DECORATION: Decoration hidden - tiledChildren: " +
                    params.tiledChildren.length +
                    ", stackedHeight: " +
                    params.stackedHeight
                );
              ...
              if (child.tab) {
                // Check if tab was disposed and recreate if needed
                if (isDisposed(child.tab)) {
                  Logger.warn("DEBUG_TAB: Tab was disposed, recreating");
                  child.tab = null;
                  child._createWindowTab();
                }

                Logger.info(
                  "DEBUG_TAB: Attempting to add tab to decoration - tab exists: " + !!child.tab
                );
                try {
                  if (child.tab && !decoration.contains(child.tab)) {
                    decoration.add_child(child.tab);
                    Logger.info("DEBUG_TAB: Tab added to decoration successfully");
                  } else if (child.tab) {
                    Logger.info("DEBUG_TAB: Tab already in decoration");
                  }
                } catch (tabError) {
                  Logger.warn("DEBUG_TAB: Failed to add tab to decoration: " + tabError.message);
                  child.tab = null;
                }
              } else {
                Logger.warn("DEBUG_TAB: No tab widget available for child window");
              }
            } catch (decorationError) {
              Logger.warn(
                "DEBUG_DECORATION: Decoration operation failed: " + decorationError.message
              );
              node.decoration = null;
            }
          } else {
            Logger.warn("DEBUG_DECORATION: Decoration is null or undefined");
          }
